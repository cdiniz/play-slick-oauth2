...
class ApplicationSpec extends PlaySpec with OneAppPerTest {
  ...
  "OAuthController" should {
    ...
    "return a valid auth token and refresh token with valid credentials" in {
      val resources = route(app, FakeRequest(POST, "/oauth/access_token").withFormUrlEncodedBody("client_id" -> "bob_client_id", "client_secret" -> "bob_client_secret", "grant_type" -> "client_credentials")).get
